...
export default class JoystickModule extends cc.EventTarget {
    ...
    public constructor(mainView: fgui.GComponent) {
        ...
        this._touchArea = mainView.getChild("JoystickTouch");
        this._center = mainView.getChild("JoystickCenter");
    ...
    private onTouchDown(evt: fgui.Event) {
        if (this._touchId == -1) {//First touch
            ...
            var bx: number = this._center.x + this._center.width / 2;
            var by: number = this._center.y + this._center.height / 2;
